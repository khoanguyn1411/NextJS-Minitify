...
export namespace ZodUtils {
  ...
  export function requiredString(message?: string): z.ZodString {
    return string({
      required_error: message ?? APP_ERROR_MESSAGES.required,
    }).min(1, message ?? APP_ERROR_MESSAGES.required);
  }

  /** Required number option. */
  export function requiredNumber(): z.ZodEffects<z.ZodNumber, number, number> {
    return number({ required_error: APP_ERROR_MESSAGES.required }).refine(
      (value) => value != null,
  ...
  const US_PHONE_VALIDATION_ERROR_MESSAGE =
  ...
  export function validUSPhoneNumber() {
    return string().regex(
      ...
      US_PHONE_VALIDATION_ERROR_MESSAGE,
    );
  }
}
